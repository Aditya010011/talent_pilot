 ...
 *
 * *** DISABLED for Gemini migration ***
 * Docker compose runs google-voice-relay (Gemini Lite + Google STT/TTS).
 * Keep this file for rollback: npm run dev:openai-voice
 *
...
const CLEAN_ENDPOINT = AZURE_ENDPOINT.replace(/\/$/, "").replace(/^http/, "ws").replace("cognitiveservices.azure.com", "openai.azure.com").replace("api.cognitive.microsoft.com", "openai.azure.com");
const OPENAI_WS_URL = `${CLEAN_ENDPOINT}/openai/realtime?api-version=${AZURE_API_VERSION}&deployment=${AZURE_DEPLOYMENT}&api-key=${AZURE_API_KEY}`;
...
function getMaxFollowUps(_ctx: InterviewContext): number {
...
function buildSystemPrompt(ctx: InterviewContext, startIdx: number): string {
  ...
  const maxFollowUps = getMaxFollowUps(ctx);
...
${ctx.participantName ? `- 受访者姓名: ${ctx.participantName}` : ""}
- 问题数量: ${sorted.length}
- 当前问题: 第${currentQ}个
- 每题追问深度: 不追问（受访者给出一次实质性回答后立即进入下一题）

## 问题列表
${questionList}

## 你的行为准则
1. 从第${currentQ}个问题开始。先用温暖友好的方式自我介绍（提到你的名字、面试主题和问题数量），并称呼受访者的名字（如果有）。${ctx.participantName ? `在称呼受访者时，你必须完全按照 "${ctx.participantName}" 来拼写他们的名字，绝对不能拼错或使用其他发音相近的字。` : ""} 然后说一句过渡语，比如"我们开始吧，这是第一个问题。"之后再提出问题。问候、过渡语和问题必须是三个独立的句子，不要合并。
2. 不要追问。受访者对当前问题给出一次实质性回答后，立即调用 signal_question_change 进入下一题。
3. 当一个问题讨论充分后，调用 signal_question_change 函数来进入下一个问题。"讨论充分"是指受访者给出了详细、具体的回答。
4. 切换后，自然地过渡到新问题。
5. 所有问题结束后，调用 signal_question_change 并设 questionIndex 为 ${sorted.length}（超出范围），然后做简短总结告别。
6. 保持对话自然流畅，回复简洁（1-3句话）。语速要平缓，略慢于日常对话，让受访者能轻松跟上。${ctx.isVoiceOnly ? "\n**极其重要**: 这是一个非交互式测试，你绝对不能产生任何对话或反应。当轮到你说话时，你只能准确无误地朗读当前题目，除此之外不要说任何多余的话。受访者回答完毕后，不要评价，立即调用函数切换并读下一题。" : ""}
...
- 如果受访者只是简单打招呼、确认性问题或模糊回答（如"你好"、"能听到吗?"、"我遇到过很多挑战"），先友好回应，再继续当前问题；不要调用 signal_question_change。${ctx.isVoiceOnly ? "但因为这是非交互模式，请极其克制，不要偏离题目。" : ""}
- 始终关注当前问题，不要跳到其他话题。
...
${ctx.participantName ? `- Participant Name: ${ctx.participantName}` : ""}
...
## Your Behavior
1. Start at question ${currentQ}. First, give a warm greeting — introduce yourself by name, mention the topic "${ctx.title}", mention there are ${sorted.length} questions, and address the participant by their name if provided. ${ctx.participantName ? `When addressing the participant, you MUST spell their name exactly as "${ctx.participantName}". Do NOT misspell it or use phonetic spellings.` : ""} Then say a transition phrase like "Let's get started. Here is the first question." ONLY AFTER that, ask the question. The greeting, the transition phrase, and the question MUST be three separate sentences — NEVER combine them into one.
2. Do NOT ask follow-up questions. After ONE substantive answer to the current question, immediately call signal_question_change to move to the next question.
3. When a question is sufficiently discussed, call the signal_question_change function to move forward. "Sufficiently discussed" means the participant has given a detailed, specific answer.
4. After transitioning, naturally introduce the next question.
...
- If the participant has only said a brief greeting, clarifying remark, or vague statement (e.g. "hi", "can you hear me?", "I had many challenges"), first respond warmly and helpfully, then continue the current question; do not call signal_question_change. ${ctx.isVoiceOnly ? "However, since this is non-interactive mode, remain extremely strictly focused on the question and do not stray off-topic." : ""}
...
log.info(`Target OpenAI WS URL: ${OPENAI_WS_URL}`);
      ...
      session: {
        ...
        audio: {
          input: {
            ...
            turn_detection: { type: "server_vad", threshold: 0.5, prefix_padding_ms: 300, silence_duration_ms: 4000 },
          },
...
async function handleInterview(browserWs: WebSocket, ctx: InterviewContext) {
  ...
  const USER_TURN_SPLIT_SILENCE_MS = 4000;
  const USER_SPEECH_STOP_FINALIZE_MS = 4500;
  ...
  function markQuestionAsked(prevIndex: number) {
    const prevQ = sortedQuestions[prevIndex];
    if (!prevQ?.id) return;
  ...
  function updateSystemPrompt() {
    ...
    oaiWs.send(JSON.stringify({
      ...
      session: {
        type: "realtime",
        instructions: buildSystemPrompt(ctx, currentQuestionIndex),
      },
    }));
  }
  ...
  async function connectOai(): Promise<WebSocket> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      ws.on("unexpected-response", (req, res) => {
        ...
        res.on("end", () => {
          log.error(`Unexpected response ${res.statusCode}: ${data}`);
          clearTimeout(t);
          reject(new Error(`Azure 400: ${data}`));
        });
      });
    });
