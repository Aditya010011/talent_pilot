...
} from "../src/lib/voice-clarification";

/** Spoken reply for repeat/clarify turns — always includes the bank question. */
export function buildClarificationRestateSpeech(
  ...
  if (!exact) {
    if (code === "zh") return "好的。";
    if (code === "yue") return "好。";
    if (code === "en") return "Sure.";
    return "";
  }
  if (isVoiceOnly) return exact;
  if (code === "en") return `Sure. ${exact}`;
  if (code === "zh") return `好的。${exact}`;
  if (code === "yue") return `好。${exact}`;
  return exact;
}

/** @deprecated Prefer isCandidateClarificationUtterance */
export { isCandidateClarificationUtterance as isMetaClarificationOnlyUtterance } from "../src/lib/voice-clarification";
