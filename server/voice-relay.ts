...
import { getGoogleTtsConfig } from "../src/lib/languages";
...
function buildTTSOptions(language?: string): TTSOptions | undefined {
  const { voiceName } = getGoogleTtsConfig(language);
...
async function handleBrowserConnection(browserWs: WebSocket, ctx: InterviewContext) {
  ...
  async function generateControlledResponse(opts?: { forceSkip?: boolean }): Promise<string> {
    ...
    const effectiveMaxFollowUps = maxFollowUps;
    ...
    if (forceSkip) {
      ...
      // No follow-ups: after any substantive exchange, wrap and advance.
      followUpInstruction =
        userTurnsOnCurrentQ >= 1
          ? bt(isZh, PROMPTS.followUp.pastLimit(NEXT_TOKEN))
          : bt(isZh, PROMPTS.followUp.pastLimit(NEXT_TOKEN));
    } else if (turnsLeft <= -1 || turnsLeft <= 0) {
      // maxFollowUps is 0 → always hard-advance after the first answer (no soft probe).
