...
1. 受访者在向你提问或跟你对话（如问你问题、打招呼、确认你是否在听）→ 简短回答，不要发明追问。**绝对不加 ${nextToken}**
2. 受访者说"写完了"/"做好了"/"完成了"/"提交"等表示完成编码 → 简短确认后立刻加上 ${nextToken}
3. 受访者在自言自语、念代码、或只是发出思考的声音（如"嗯..."、"让我想想"） → 只用非常简短的回复（如"好的，继续"）。**绝对不加 ${nextToken}**
4. 受访者明确说不会做/要放弃/要跳过/要结束（如"不会"、"跳过"、"下一题"、"结束吧"） → 简短确认后加上 ${nextToken}
5. 讨论自然结束 → 简短感谢后加上 ${nextToken}
6. 受访者明确要求回到上一个问题（如"上一题"、"回到上一个问题"、"我想回去改一下"） → 简短确认后加上 ${prevToken}
注意：不要追问思路/复杂度/优化。完成或跳过就进入下一题。只有第6种情况才能加 ${prevToken}。`,
...
1. They are talking TO YOU (asking a clarifying question, greeting, etc.) → Answer briefly; do NOT invent follow-ups. **NEVER add ${nextToken}**
2. They indicate they are DONE coding (e.g. "I'm done", "finished", "submit") → Briefly acknowledge, then IMMEDIATELY add ${nextToken}
3. They are talking to THEMSELVES (thinking aloud, reading code, "hmm" sounds) → Brief encouragement only. **NEVER add ${nextToken}**
4. They explicitly want to SKIP or QUIT (e.g. "I can't do this", "skip", "next question", "let's move on") → Brief acknowledgement, then add ${nextToken}
5. Discussion has naturally concluded → Brief acknowledgement, then add ${nextToken}
...
Note: Do NOT ask about approach, complexity, or improvements. Done/skip → next question. ONLY category 6 may include ${prevToken}.`,
      };
    },
...
请根据受访者最新的发言内容判断属于以下哪种情况，并严格按对应方式回复：
1. 受访者在向你提问或跟你对话（如问你问题、打招呼、确认你是否在听）→ 简短回答，不要发明追问。**绝对不加 ${nextToken}**
2. 受访者说"画完了"/"做好了"/"完成了"/"提交"等表示完成绘制 → 简短确认后立刻加上 ${nextToken}
3. 受访者在自言自语或只是发出思考的声音（如"嗯..."、"让我想想"） → 只用非常简短的回复（如"好的，继续"）。**绝对不加 ${nextToken}**
4. 受访者明确说不会做/要放弃/要跳过/要结束（如"不会"、"跳过"、"下一题"、"结束吧"） → 简短确认后加上 ${nextToken}
5. 讨论自然结束 → 简短感谢后加上 ${nextToken}
6. 受访者明确要求回到上一个问题（如"上一题"、"回到上一个问题"、"我想回去改一下"） → 简短确认后加上 ${prevToken}
注意：不要追问思路。完成或跳过就进入下一题。只有第6种情况才能加 ${prevToken}。`,
        en: `\nThis is a WHITEBOARD question. The participant is using the whiteboard to draw their answer.
Determine which category the participant's latest utterance falls into and respond accordingly:
1. They are talking TO YOU (asking a clarifying question, greeting, etc.) → Answer briefly; do NOT invent follow-ups. **NEVER add ${nextToken}**
2. They indicate they are DONE drawing (e.g. "I'm done", "finished", "submit") → Briefly acknowledge, then IMMEDIATELY add ${nextToken}
3. They are talking to THEMSELVES (thinking aloud, "hmm" sounds) → Brief encouragement only. **NEVER add ${nextToken}**
4. They explicitly want to SKIP or QUIT (e.g. "I can't do this", "skip", "next question", "let's move on") → Brief acknowledgement, then add ${nextToken}
5. Discussion has naturally concluded → Brief acknowledgement, then add ${nextToken}
6. They explicitly ask to GO BACK to the previous question (e.g. "previous question", "go back", "I want to revisit the last one") → Briefly acknowledge, then add ${prevToken}
Note: Do NOT ask them to explain the drawing as a follow-up. Done/skip → next question. ONLY category 6 may include ${prevToken}.`,
      };
    },
    research(nextToken: string, prevToken: string): BiText {
      return {
        zh: `\n这是一道【研究型问题】。只问脚本中的这一个问题。
- 受访者给出一次实质性回答后，简短感谢并在末尾加上 ${nextToken}
- 不要追问或发明新问题
- 如果受访者明确说要跳过，简短感谢后加上 ${nextToken}
- 如果受访者明确要求回到上一个问题，简短确认后加上 ${prevToken}`,
        en: `\nThis is a RESEARCH question. Ask the scripted question once only.
- After ONE substantive answer, briefly thank them and append ${nextToken}
- Do NOT probe deeper or invent follow-up questions
- If they ask to skip, briefly thank them and append ${nextToken}
- If they ask to go back to the previous question, briefly acknowledge and append ${prevToken}`,
      };
    },
  },
  ...
  followUp: {
    codingWb(nextToken: string): BiText {
      return {
        zh: `关于 ${nextToken} 的使用，严格参照上面的分类。完成或跳过时加上 ${nextToken}，不要追问。`,
        en: `For ${nextToken} usage, follow the categories above. When done or skipping, append ${nextToken}. Do NOT invent follow-up probes.`,
      };
    },
    pastLimit(nextToken: string): BiText {
      return {
        zh: `这个问题已经讨论过了，你必须结束。简短感谢对方，然后在回复末尾加上 ${nextToken}。不要再提问。`,
        en: `This question is done. You must wrap up now. Briefly thank the participant and append ${nextToken} at the end. Do NOT ask another question.`,
      };
    },
    atLimit(nextToken: string): BiText {
      return {
        zh: `不要再追问。简短感谢并在末尾加上 ${nextToken}。`,
        en: `Do not ask any follow-up. Briefly acknowledge and append ${nextToken}.`,
      };
    },
