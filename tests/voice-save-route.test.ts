...
test("handleVoiceSave does not rewrite timing when session already COMPLETED", async () => {
  const { ops, updatedSessions, summaryCalls, infoLogs } = createOps();
  const alreadyDoneOps = {
    ...ops,
    async loadSessionForCompletion(sessionId: string) {
      void sessionId;
      return {
  ...
  const result = await handleVoiceSave(
    {
      ...
      messages: [{ role: "user", content: "late flush", questionId: "q-1" }],
      complete: true,
  ...
  assert.deepEqual(updatedSessions[0], {
    sessionId: "session-already-done",
    payload: { tokenUsage: { promptTokens: 1, completionTokens: 2, totalTokens: 3 } },
  });
  assert.equal(summaryCalls.length, 0);
  assert.ok(
    infoLogs.some((message) =>
      message.includes("already COMPLETED — skipped completion rewrite"),
    ),
  );
});
