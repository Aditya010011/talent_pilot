...
test("Production browser defaults use same-origin relay paths instead of raw relay ports", () => {
  const targets = buildRelayTargets({
    ...
    browserHost: "kimiyi-ai.com",
  });

  assert.equal(targets[0].url, "wss://kimiyi-ai.com/ws/voice");
  assert.equal(targets[1].url, "wss://kimiyi-ai.com/ws/openai-voice");
});

test("Same-origin voice relay URL derives a same-origin OpenAI relay path", () => {
  const urls = resolveRelayUrls({
    voiceRelayUrl: "wss://kimiyi-ai.com/ws/voice",
  });

  assert.equal(urls.voiceRelayUrl, "wss://kimiyi-ai.com/ws/voice");
  assert.equal(urls.openAiRelayUrl, "wss://kimiyi-ai.com/ws/openai-voice");
});
