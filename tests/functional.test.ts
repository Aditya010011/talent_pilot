...
test("login honors browser locale and persisted locale cache", async () => {
  ...
  await cachedPage.addInitScript(() => {
    window.localStorage.setItem("kimiyi.app.locale", "zh");
  });
...
test("mobile auth callback page hands OAuth params back to the app", async () => {
  ...
  await waitForText(page, "Returning you to Kimiyi AI");

  const expected =
    "com.kimiyi.mobile://login-callback?code=oauth-code&state=xyz";

  await waitForCondition(async () => {
    const href = await page
      .getByRole("link", { name: "Open Kimiyi AI" })
      .getAttribute("href");
    return href === expected;
  });
  assert.equal(
    await page.getByRole("link", { name: "Open Kimiyi AI" }).getAttribute("href"),
    expected,
  );
