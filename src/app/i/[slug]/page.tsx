...
export default function PublicInterviewPage() {
  ...
  useEffect(() => {
    if (!interview.data || chosenLanguage) return;
    setChosenLanguage(resolveLanguage(interview.data.language).code);
  }, [interview.data, chosenLanguage]);
  ...
  const createSession = trpc.session.create.useMutation({
    ...
    onError: (err) => {
      toast({
        title: t("publicInterview.failedToStart"),
      ...
      <div className="flex min-h-screen items-center justify-center bg-muted/30 p-4">
        <Card className="w-full max-w-md">
          <CardContent className="py-12 text-center">
            ...
            <h2 className="text-xl font-semibold">{t("publicInterview.notAvailable")}</h2>
            <p className="text-muted-foreground mt-2">
              {t("publicInterview.notAvailableDesc")}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="flex min-h-screen items-center justify-center bg-muted/30 p-4">
        <Card className="w-full max-w-md">
          <CardContent className="py-12 text-center">
            ...
            <h2 className="mt-4 text-2xl font-bold">{t("publicInterview.completed")}</h2>
            <p className="mt-2 text-muted-foreground">
              {t("publicInterview.completedDesc")}
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent>
          ...
          {interview.data.requireInvite && !isPreview && !canResume && (
            <div className="py-6 text-center">
              ...
              <p className="mt-3 font-medium">{t("publicInterview.inviteOnly")}</p>
              <p className="mt-1 text-sm text-muted-foreground">
                {t("publicInterview.inviteOnlyDesc")}
              </p>
            </div>
          )}
            ...
            <div className="mb-6 space-y-3">
              <div className="rounded-lg border border-primary/30 bg-primary/5 p-4">
                <p className="text-sm font-medium">
                  {t("publicInterview.unfinishedSession")}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("publicInterview.unfinishedSessionDesc")}
                </p>
                <div className="mt-3 flex gap-2">
                  <Button className="flex-1" onClick={handleResume}>
                    <RotateCcw className="mr-2 h-4 w-4" />
                    {t("publicInterview.continueInterview")}
                  </Button>
                  <Button variant="outline" className="flex-1" onClick={handleStartNew}>
                    <Plus className="mr-2 h-4 w-4" />
                    {t("publicInterview.startNew")}
                  </Button>
                </div>
              </div>
            </div>
          ...
          {(!interview.data.requireInvite || isPreview) && !canResume && (
            ...
            >
              <div className="space-y-2">
                <Label htmlFor="name">
                  {t("publicInterview.yourName")} <span className="text-destructive">*</span>
                </Label>
                <Input
                  id="name"
                  value={participantName}
                  onChange={(e) => setParticipantName(e.target.value)}
                  placeholder={t("publicInterview.yourNamePlaceholder")}
                  required
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="email">
                  {t("publicInterview.yourEmail")} <span className="text-destructive">*</span>
                </Label>
                <Input
                  id="email"
                  type="email"
                  ...
                  placeholder={t("publicInterview.yourEmailPlaceholder")}
                  required
                />
              </div>
              ...
              {showLanguagePicker && (
                <CandidateLanguageSelect
                  value={chosenLanguage ?? resolveLanguage(interview.data.language).code}
                  onChange={(code) => {
                    setChosenLanguage(code);
                  }}
                  label={t("publicInterview.chooseLanguage")}
                  description={t("publicInterview.chooseLanguageDesc")}
                />
              )}

              <div className="rounded-lg bg-muted p-4 text-sm text-muted-foreground">
                <p>
                  {interview.data.questions.length} {t("publicInterview.questions")} &middot;{" "}
                  {interview.data.timeLimitMinutes
                    ? `${interview.data.timeLimitMinutes} ${t("publicInterview.min")}`
                    : t("publicInterview.noTimeLimit")}
                  ...
                  <>
                    ...
                    <span>
                      {interview.data.chatEnabled
                        ? t("publicInterview.voiceAndTextChat")
                        : t("publicInterview.voiceModeOnly")}
                    </span>
                  </>
                ) : (
                  <>
                    <MessageSquare className="h-4 w-4 text-primary" />
                    <span>{t("publicInterview.textChatOnly")}</span>
                  </>
                )}
              </div>
              ...
              >
                ...
                {t("publicInterview.beginInterview")}
              </Button>
            </form>
          )}
        </CardContent>
