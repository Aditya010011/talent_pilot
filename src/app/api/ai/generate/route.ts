...
import { trimQuestionsToDuration } from "@/lib/interview-duration";
...
export async function POST(req: Request) {
  ...
  const { description, durationMinutes, language, jobDescription, resumeText, existingCriteria, isVoiceOnly, codingQuestions, behavioralQuestions, codingLanguage, numQuestions } = await req.json();
  if (!description) {
    return new Response(JSON.stringify({ error: "Description is required" }), { status: 400 });
  }

  const provider = getProvider(GENERATOR_MODEL);
  const messages = buildGeneratorPrompt(description, durationMinutes, language, jobDescription, resumeText, existingCriteria, Boolean(isVoiceOnly), codingQuestions, behavioralQuestions, codingLanguage, numQuestions);
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        if (Array.isArray(generated.questions) && !(typeof numQuestions === "number" && numQuestions > 0) && !codingQuestions && !behavioralQuestions) {
          generated.questions = trimQuestionsToDuration(
            generated.questions,
            durationMinutes,
          );
        }
