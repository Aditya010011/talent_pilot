...
export async function POST(req: Request) {
  ...
  const { interview, feedback, language, jobDescription, resumeText, isVoiceOnly, codingLanguage } = await req.json();
  ...
  const messages = buildImprovePrompt(interview, feedback, language, jobDescription, resumeText, Boolean(isVoiceOnly), codingLanguage);
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        enqueue(sse({ type: "done", data: refined }));
      } catch (error) {
        log.error("Interview refinement error:", error);
        enqueue(sse({ type: "error", message: "Failed to refine interview" }));
      } finally {
        close();
      }
    },
