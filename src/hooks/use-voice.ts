"use client";

import { createLogger } from "@/lib/logger";
import { isCandidateClarificationUtterance } from "@/lib/voice-clarification";
import {
  buildRelayTargets,
  isRecoverableRelayErrorMessage,
  RelayConnector,
  relayDisplayName,
  resolveRelayPrimaryPreference,
} from "@/lib/voice/relay-routing";
import {
  JITTER_BUFFER_MAX_WAIT_MS,
  PLAYBACK_SAMPLE_RATE,
  shouldFlushPlaybackQueue,
} from "@/lib/voice/playback-jitter-buffer";
import { useCallback, useEffect, useRef, useState } from "react";

const log = createLogger("voice");

export interface InterviewContext {
  title: string;
  participantName?: string | null;
  objective?: string | null;
  aiName: string;
  aiTone: string;
  language: string;
  followUpDepth: string;
  isVoiceOnly?: boolean;
  startQuestionIndex?: number;
  questionsAsked?: string[];
  timeLimitMinutes?: number | null;
  /** STT-only mode — no Gemini TTS/greeting (Vidu owns speaking). */
  skipLlm?: boolean;
  questions: Array<{
    id?: string;
    text: string;
    type: string;
    description?: string | null;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    options?: any;
    starterCode?: { language: string; code: string; templates?: Record<string, string> } | null;
    order: number;
  }>;
}

interface UseVoiceOptions {
  interviewId: string;
  sessionId: string;
  interviewContext: InterviewContext;
  onTranscript?: (text: string, isFinal: boolean) => void;
  onAIResponse?: (text: string) => void;
  onError?: (error: string) => void;
  onQuestionChange?: (index: number, total: number) => void;
  onTtsChunk?: (pcmData: ArrayBuffer) => void;
  onInterrupt?: () => void;
  commitTurn?: () => Promise<string>;
  /** Raw mic PCM while listening (16-bit LE mono @ 16kHz). Used for push-to-send avatars. */
  onMicPcmChunk?: (pcm: Int16Array) => void;
  /** Called when listening starts (clear utterance buffers). */
  onListeningStart?: () => void;
  /** When true, suppress browser AudioContext playback (e.g. avatar handles audio) */
  mutePlayback?: boolean;
  /** When true, tell the relay to skip Gemini greeting/TTS/replies (STT only). */
  skipLlm?: boolean;
}

interface VoiceState {
  isConnected: boolean;
  isListening: boolean;
  isSpeaking: boolean;
  isProcessing: boolean;
  isTransitioning: boolean;
  transitionDirection: "next" | "previous" | null;
  isSaving: boolean;
  isInterviewComplete: boolean;
  userTranscript: string;
  aiTranscript: string;
  lastAiQuestion: string;
  lastAssistantUtteranceEndedAt: number;
  audioLevel: number;
  currentQuestionIndex: number;
  totalQuestions: number;
  tokenUsage?: { promptTokens: number; completionTokens: number; totalTokens: number } | null;
}

interface TrackedMessage {
  role: "user" | "assistant";
  content: string;
  source?: "voice" | "chat";
  questionIndex?: number;
  id?: string;
  timestamp?: string;
}

/**
 * Voice interview hook using Volcengine S2S (Speech-to-Speech) via relay.
 *
 * Flow:
 * 1. Browser connects and sends interview context to relay
 * 2. Relay builds system prompt and connects to Volcengine
 * 3. Browser captures mic audio as 16kHz int16 PCM
 * 4. Audio sent to relay server via WebSocket (hex-encoded)
 * 5. Relay forwards to Volcengine S2S which handles ASR + LLM + TTS
 * 6. TTS audio (24kHz float32 PCM) streamed back and played via AudioContext
 * 7. Per-question transitions managed by relay with LLM summarization
 * 8. On disconnect, all messages are saved to database
 */
export function useVoice({
  sessionId,
  interviewContext,
  onTranscript,
  onAIResponse,
  onError,
  onQuestionChange,
  onTtsChunk,
  onInterrupt,
  onMicPcmChunk,
  onListeningStart,
  mutePlayback = false,
  skipLlm = false,
}: UseVoiceOptions) {
  // Use a ref so onBinaryMessage closure always reads the latest value
  const mutePlaybackRef = useRef(mutePlayback);
  useEffect(() => { mutePlaybackRef.current = mutePlayback; }, [mutePlayback]);
  /** When speakers are muted (Simli), still track TTS duration for isSpeaking. */
  const muteSpeakUntilRef = useRef(0);
  const muteSpeakTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const skipLlmRef = useRef(skipLlm);
  useEffect(() => { skipLlmRef.current = skipLlm; }, [skipLlm]);
  const onMicPcmChunkRef = useRef(onMicPcmChunk);
  useEffect(() => { onMicPcmChunkRef.current = onMicPcmChunk; }, [onMicPcmChunk]);
  const onListeningStartRef = useRef(onListeningStart);
  useEffect(() => { onListeningStartRef.current = onListeningStart; }, [onListeningStart]);
  const onTtsChunkRef = useRef(onTtsChunk);
  useEffect(() => { onTtsChunkRef.current = onTtsChunk; }, [onTtsChunk]);
  const [state, setState] = useState<VoiceState>({
    isConnected: false,
    isListening: false,
    isSpeaking: false,
    isProcessing: false,
    isTransitioning: false,
    transitionDirection: null,
    isSaving: false,
    isInterviewComplete: false,
    userTranscript: "",
    aiTranscript: "",
    lastAiQuestion: "",
    lastAssistantUtteranceEndedAt: 0,
    audioLevel: 0,
    currentQuestionIndex: interviewContext.startQuestionIndex ?? 0,
    totalQuestions: interviewContext.questions.length,
    tokenUsage: null,
  });

  const relayConnectorRef = useRef<RelayConnector<Record<string, unknown>> | null>(null);
  const audioContextRef = useRef<AudioContext | null>(null);
  const mediaStreamRef = useRef<MediaStream | null>(null);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const processorRef = useRef<any>(null);
  const playTimeRef = useRef(0);
  const audioSourcesRef = useRef<AudioBufferSourceNode[]>([]);
  const queuedAudioChunksRef = useRef<Float32Array[]>([]);
  const queuedAudioSamplesRef = useRef(0);
  const firstQueuedAudioAtRef = useRef<number | null>(null);
  const playbackFlushTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null
  );
  const heartbeatIntervalRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const isListeningRef = useRef(false);
  const listeningTurnStartedAtRef = useRef<string | null>(null);
  const trackedMessagesRef = useRef<TrackedMessage[]>([]);
  const micHoldUntilRef = useRef(0);
  const bargeInFramesRef = useRef(0);
  const tokenUsageRef = useRef<{ promptTokens: number; completionTokens: number; totalTokens: number } | null>(null);

  const BARGE_IN_RMS_THRESHOLD = 0.02;
  const BARGE_IN_FRAME_COUNT = 2;

  const onInterruptRef = useRef(onInterrupt);
  useEffect(() => { onInterruptRef.current = onInterrupt; }, [onInterrupt]);
  const lastFinalUserTranscriptRef = useRef<{ text: string; at: number } | null>(null);

  // Buffers for accumulating streaming chunks
  const asrBufferRef = useRef<string>("");
  const chatBufferRef = useRef<string>("");
  const lastOnAIResponseRef = useRef<string>("");
  const saveInFlightRef = useRef(false);
  const pendingSaveRef = useRef(false);
  const saveDebounceTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const relayKeepaliveTimerRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const currentQuestionIndexRef = useRef(
    interviewContext.startQuestionIndex ?? 0
  );
  /** Question index for the in-flight assistant turn (set on user commit, not on question_change). */
  const activeTurnQuestionIndexRef = useRef(
    interviewContext.startQuestionIndex ?? 0
  );
  /** Client-side guard: block UI advances after a repeat/clarify turn. */
  const lastUserWasClarificationRef = useRef(false);
  const latestCodeUpdateRef = useRef<{ content: string; language: string } | null>(
    null
  );
  const latestWhiteboardUpdateRef = useRef<string | null>(null);

  useEffect(() => {
    currentQuestionIndexRef.current = state.currentQuestionIndex;
  }, [state.currentQuestionIndex]);

  // Cleanup on unmount
  useEffect(() => {
    return () => {
      cleanup();
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const cleanup = useCallback(() => {
    stopListening();
    interruptPlayback();
    if (heartbeatIntervalRef.current) {
      clearInterval(heartbeatIntervalRef.current);
      heartbeatIntervalRef.current = null;
    }
    if (relayKeepaliveTimerRef.current) {
      clearInterval(relayKeepaliveTimerRef.current);
      relayKeepaliveTimerRef.current = null;
    }
    if (saveDebounceTimerRef.current) {
      clearTimeout(saveDebounceTimerRef.current);
      saveDebounceTimerRef.current = null;
    }
    relayConnectorRef.current?.close();
    relayConnectorRef.current = null;
    audioContextRef.current?.close();
    audioContextRef.current = null;
    setState((prev) => ({
      ...prev,
      isConnected: false,
      isListening: false,
      isSpeaking: false,
      isProcessing: false,
      isTransitioning: false,
      transitionDirection: null,
      userTranscript: "",
      aiTranscript: "",
      lastAiQuestion: "",
      lastAssistantUtteranceEndedAt: 0,
    }));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const pushTrackedMessage = useCallback((msg: Omit<TrackedMessage, "id" | "questionIndex"> & { questionIndex?: number }) => {
    trackedMessagesRef.current.push({
      ...msg,
      id: crypto.randomUUID(),
      timestamp: msg.timestamp ?? new Date().toISOString(),
      questionIndex: msg.questionIndex ?? currentQuestionIndexRef.current,
    });
  }, []);

  /** Sorted interview questions (same order the relay uses). */
  const getSortedQuestions = useCallback(() => {
    return [...interviewContext.questions].sort((a, b) => a.order - b.order);
  }, [interviewContext.questions]);

  /** Accumulate all speech within one unmute→mute turn into a single answer. */
  const listeningTurnTextRef = useRef("");
  const isInListeningTurnRef = useRef(false);
  /** True after commitTurn until asr_ended (or timeout) — forces persist path even if mic still open. */
  const commitPendingRef = useRef(false);
  /** Resolves when relay returns asr_ended for an in-flight commitTurn (or timeout). */
  const commitTurnWaiterRef = useRef<{
    resolve: (text: string) => void;
    timer: ReturnType<typeof setTimeout>;
  } | null>(null);
  const COMMIT_TURN_TIMEOUT_MS = 2800;

  const resolveCommitTurnWaiter = useCallback((text: string) => {
    const waiter = commitTurnWaiterRef.current;
    if (!waiter) return;
    clearTimeout(waiter.timer);
    commitTurnWaiterRef.current = null;
    commitPendingRef.current = false;
    waiter.resolve(text);
  }, []);

  /** Set clarification guard as early as possible (commitTurn + asr_ended). */
  const markClarificationFromUserText = useCallback((text: string, source: "commit" | "asr") => {
    const isClarification = isCandidateClarificationUtterance(text);
    lastUserWasClarificationRef.current = isClarification;
    if (isClarification) {
      log.info(
        `Client clarification detected at ${source} — will ignore advance until next answer: "${text.slice(0, 80)}"`,
      );
    }
    return isClarification;
  }, []);

  const scheduleSaveProgressRef = useRef<(idx: number) => void>(() => {});

  const clearPlaybackFlushTimer = useCallback(() => {
    if (playbackFlushTimerRef.current) {
      clearTimeout(playbackFlushTimerRef.current);
      playbackFlushTimerRef.current = null;
    }
  }, []);

  const clearQueuedAudio = useCallback(() => {
    clearPlaybackFlushTimer();
    queuedAudioChunksRef.current = [];
    queuedAudioSamplesRef.current = 0;
    firstQueuedAudioAtRef.current = null;
  }, [clearPlaybackFlushTimer]);

  const clearMuteSpeakTimer = useCallback(() => {
    if (muteSpeakTimerRef.current) {
      clearTimeout(muteSpeakTimerRef.current);
      muteSpeakTimerRef.current = null;
    }
    muteSpeakUntilRef.current = 0;
  }, []);

  /** Stop all currently playing audio sources and notify recording mixer */
  const interruptPlayback = useCallback(() => {
    clearQueuedAudio();
    clearMuteSpeakTimer();
    for (const source of audioSourcesRef.current) {
      try {
        source.stop();
      } catch {
        // already stopped
      }
    }
    audioSourcesRef.current = [];
    playTimeRef.current = 0;
    micHoldUntilRef.current = performance.now() + 250;
    bargeInFramesRef.current = 0;
    setState((s) => ({ ...s, isSpeaking: false }));
    onInterruptRef.current?.();
  }, [clearQueuedAudio, clearMuteSpeakTimer]);

  const scheduleQueuedAudioFlush = useCallback(
    (flushQueuedAudio: (force?: boolean) => void) => {
      if (playbackFlushTimerRef.current) return;
      const firstQueuedAt = firstQueuedAudioAtRef.current;
      const delayMs =
        typeof firstQueuedAt === "number"
          ? Math.max(0, JITTER_BUFFER_MAX_WAIT_MS - (performance.now() - firstQueuedAt))
          : JITTER_BUFFER_MAX_WAIT_MS;
      playbackFlushTimerRef.current = setTimeout(() => {
        playbackFlushTimerRef.current = null;
        flushQueuedAudio(true);
      }, delayMs);
    },
    []
  );

  const flushQueuedAudio = useCallback((force = false) => {
    const ctx = audioContextRef.current;
    if (!ctx) return;
    if (queuedAudioSamplesRef.current === 0) return;

    const now = ctx.currentTime;
    const bufferedAheadMs = Math.max(0, (playTimeRef.current - now) * 1000);
    if (
      !force &&
      !shouldFlushPlaybackQueue({
        queuedSamples: queuedAudioSamplesRef.current,
        bufferedAheadMs,
        firstChunkQueuedAtMs: firstQueuedAudioAtRef.current,
        nowMs: performance.now(),
        sampleRate: PLAYBACK_SAMPLE_RATE,
      })
    ) {
      scheduleQueuedAudioFlush(flushQueuedAudio);
      return;
    }

    clearPlaybackFlushTimer();

    const float32 = new Float32Array(queuedAudioSamplesRef.current);
    let offset = 0;
    for (const chunk of queuedAudioChunksRef.current) {
      float32.set(chunk, offset);
      offset += chunk.length;
    }

    queuedAudioChunksRef.current = [];
    queuedAudioSamplesRef.current = 0;
    firstQueuedAudioAtRef.current = null;

    const audioBuffer = ctx.createBuffer(1, float32.length, PLAYBACK_SAMPLE_RATE);
    audioBuffer.copyToChannel(float32, 0);

    const source = ctx.createBufferSource();
    source.buffer = audioBuffer;
    source.connect(ctx.destination);

    // Schedule playback sequentially
    const startAt = Math.max(now, playTimeRef.current);
    source.start(startAt);
    playTimeRef.current = startAt + audioBuffer.duration;

    const ttsChunkHandler = onTtsChunkRef.current;
    if (ttsChunkHandler) {
      const chunkCopy = float32.buffer.slice(
        float32.byteOffset,
        float32.byteOffset + float32.byteLength
      );
      const delayMs = Math.max(0, (startAt - now) * 1000);
      if (delayMs <= 0) {
        ttsChunkHandler(chunkCopy);
      } else {
        setTimeout(() => ttsChunkHandler(chunkCopy), delayMs);
      }
    }
    micHoldUntilRef.current =
      performance.now() + Math.max(0, (playTimeRef.current - now) * 1000) + 250;
    bargeInFramesRef.current = 0;

    audioSourcesRef.current.push(source);
    setState((s) => ({
      ...s,
      isSpeaking: true,
      isProcessing: false,
    }));

    source.onended = () => {
      audioSourcesRef.current = audioSourcesRef.current.filter(
        (s) => s !== source
      );
      if (
        audioSourcesRef.current.length === 0 &&
        queuedAudioSamplesRef.current === 0
      ) {
        setState((s) => ({ ...s, isSpeaking: false }));
      }
    };
  }, [clearPlaybackFlushTimer, scheduleQueuedAudioFlush]);

  /** Queue incoming float32 PCM audio chunk and flush through a small jitter buffer. */
  const playAudio = useCallback(
    (pcmData: ArrayBuffer) => {
      const ctx = audioContextRef.current;
      if (!ctx) return;
      if (ctx.state === "suspended") {
        void ctx.resume();
      }

      if (pcmData.byteLength < 4) return;
      const float32 = new Float32Array(pcmData);
      if (float32.length === 0) return;

      if (firstQueuedAudioAtRef.current === null) {
        firstQueuedAudioAtRef.current = performance.now();
      }
      queuedAudioChunksRef.current.push(float32);
      queuedAudioSamplesRef.current += float32.length;
      flushQueuedAudio(true);
    },
    [flushQueuedAudio]
  );

  const replayLatestRelayContext = useCallback(
    (connector?: RelayConnector<Record<string, unknown>> | null) => {
      const client = connector ?? relayConnectorRef.current;
      if (!client) return;

      const latestCode = latestCodeUpdateRef.current;
      if (latestCode) {
        client.sendJson({
          type: "code_update",
          content: latestCode.content,
          language: latestCode.language,
        });
      }

      const latestWhiteboard = latestWhiteboardUpdateRef.current;
      if (latestWhiteboard) {
        client.sendJson({
          type: "whiteboard_update",
          imageDataUrl: latestWhiteboard,
        });
      }
    },
    []
  );

  /** Connect to the voice relay server */
  const connect = useCallback(async () => {
    try {
      // Request microphone permission
      if (typeof navigator === "undefined" || !navigator.mediaDevices) {
        throw new Error("Microphone access requires a secure (HTTPS) connection or is not supported.");
      }
      await navigator.mediaDevices.getUserMedia({ audio: true });

      // Create AudioContext for playback
      if (!audioContextRef.current) {
        const AudioCtx = window.AudioContext || (window as any).webkitAudioContext;
        if (!AudioCtx) throw new Error("AudioContext not supported");
        audioContextRef.current = new AudioCtx({ sampleRate: 24000 });
      }
      if (audioContextRef.current.state === "suspended") {
        await audioContextRef.current.resume();
      }

      // Reset tracked messages
      trackedMessagesRef.current = [];

      relayConnectorRef.current?.close();

      const targets = buildRelayTargets({
        language: interviewContext.language,
        voiceRelayUrl: process.env.NEXT_PUBLIC_VOICE_RELAY_URL,
        openAiRelayUrl: process.env.NEXT_PUBLIC_OPENAI_VOICE_RELAY_URL,
        primaryPreference: resolveRelayPrimaryPreference(
          process.env.NEXT_PUBLIC_VOICE_RELAY_PRIMARY,
        ),
        browserProtocol:
          typeof window !== "undefined" ? window.location.protocol : undefined,
        browserHost:
          typeof window !== "undefined" ? window.location.host : undefined,
      });

      const connector = new RelayConnector<Record<string, unknown>>({
        targets,
        binaryType: "arraybuffer",
        buildInitMessage: () => ({
          type: "init",
          context: {
            ...interviewContext,
            startQuestionIndex: currentQuestionIndexRef.current,
            skipLlm: skipLlmRef.current,
          },
        }),
        onJsonMessage: (msg) => {
          handleRelayMessage(msg);
        },
        onBinaryMessage: (data) => {
          // Avatar modes speak their own audio — don't also play GCP TTS.
          // Simli still consumes these chunks for lipsync + recording.
          if (mutePlaybackRef.current) {
            onTtsChunkRef.current?.(data);
            if (data instanceof ArrayBuffer && data.byteLength >= 4) {
              const samples = Math.floor(data.byteLength / 4);
              const durationMs = (samples / PLAYBACK_SAMPLE_RATE) * 1000;
              const now = performance.now();
              muteSpeakUntilRef.current = Math.max(muteSpeakUntilRef.current, now) + durationMs;
              setState((s) => ({ ...s, isSpeaking: true, isProcessing: false }));
              micHoldUntilRef.current = muteSpeakUntilRef.current + 250;
              if (muteSpeakTimerRef.current) clearTimeout(muteSpeakTimerRef.current);
              const remaining = Math.max(0, muteSpeakUntilRef.current - now);
              muteSpeakTimerRef.current = setTimeout(() => {
                muteSpeakTimerRef.current = null;
                muteSpeakUntilRef.current = 0;
                setState((s) => ({ ...s, isSpeaking: false }));
              }, remaining);
            }
          } else {
            playAudio(data);
          }
        },
        onConnected: ({ target, isFailover, connector: activeConnector }) => {
          log.info(
            `${isFailover ? "Failed over to" : "Connected to"} ${relayDisplayName(
              target.kind
            )} @ ${target.url}`
          );
          if (isFailover) {
            replayLatestRelayContext(activeConnector);
          }
          if (heartbeatIntervalRef.current) clearInterval(heartbeatIntervalRef.current);
          heartbeatIntervalRef.current = setInterval(() => {
            activeConnector.sendJson({ type: "ping" });
          }, 30000);
          if (relayKeepaliveTimerRef.current) clearInterval(relayKeepaliveTimerRef.current);
          // Keep relay/STT alive while mic is muted (e.g. during coding)
          const silenceHex = "00".repeat(320);
          relayKeepaliveTimerRef.current = setInterval(() => {
            if (!activeConnector.isReady || isListeningRef.current) return;
            activeConnector.sendJson({ type: "audio", data: silenceHex });
          }, 4000);
        },
        onFailover: ({ from, to, reason }) => {
          log.warn(
            `Relay failover: ${relayDisplayName(from.kind)} -> ${relayDisplayName(
              to.kind
            )} (${reason})`
          );
        },
        onPermanentFailure: (error) => {
          log.error("Voice relay exhausted all targets:", error.message);
          setState((s) => ({ ...s, isConnected: false }));
          onError?.(
            error.message || "Voice connection error. Is the relay server running?"
          );
        },
      });

      relayConnectorRef.current = connector;
      await connector.connect();
    } catch (error) {
      const msg =
        error instanceof Error ? error.message : "Voice connection failed";
      onError?.(msg);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [onError, playAudio, interviewContext]);

  /** Extract text from a Volcengine event payload, trying common field names */
  const extractText = useCallback(
    (data: Record<string, unknown> | undefined): string => {
      if (!data) return "";
      for (const key of ["text", "content", "sentence", "delta"]) {
        if (typeof data[key] === "string" && data[key]) return data[key] as string;
      }
      return "";
    },
    []
  );

  /** Save accumulated messages to the server (incremental, non-completing).
   *  Does NOT flush interim ASR — only committed unmute→mute turns become user rows. */
  const flushSaveProgress = useCallback(
    async (currentQuestionIndex: number) => {
      if (saveInFlightRef.current) {
        pendingSaveRef.current = true;
        return;
      }
      saveInFlightRef.current = true;

      const messages = [...trackedMessagesRef.current];
      trackedMessagesRef.current = [];

      try {
        if (messages.length === 0 && typeof currentQuestionIndex !== "number") return;

        await fetch("/api/voice/save", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ sessionId, messages, currentQuestionIndex }),
        });
        log.info(
          `Progress saved: ${messages.length} msgs, Q${currentQuestionIndex + 1}`
        );
      } catch (err) {
        log.error("Failed to save progress:", err);
      } finally {
        saveInFlightRef.current = false;
        if (pendingSaveRef.current) {
          pendingSaveRef.current = false;
          void flushSaveProgress(currentQuestionIndexRef.current);
        }
      }
    },
    [sessionId]
  );

  const scheduleSaveProgress = useCallback(
    (currentQuestionIndex: number) => {
      if (saveDebounceTimerRef.current) {
        clearTimeout(saveDebounceTimerRef.current);
      }
      saveDebounceTimerRef.current = setTimeout(() => {
        saveDebounceTimerRef.current = null;
        void flushSaveProgress(currentQuestionIndex);
      }, 500);
    },
    [flushSaveProgress]
  );

  useEffect(() => {
    scheduleSaveProgressRef.current = scheduleSaveProgress;
  }, [scheduleSaveProgress]);

  /**
   * Record every scripted question as its own assistant transcript entry when the
   * question stage starts — including CODING/WHITEBOARD (often not spoken aloud).
   */
  const recordQuestionEntry = useCallback(
    (questionIndex: number) => {
      const q = getSortedQuestions()[questionIndex];
      if (!q?.text?.trim()) return;

      let content = q.text.trim();
      if (q.description?.trim()) {
        content = `${content}\n\n${q.description.trim()}`;
      }

      const normalized = content.replace(/\s+/g, " ").trim().toLowerCase();
      const alreadyRecorded = trackedMessagesRef.current.some((m) => {
        if (m.role !== "assistant" || m.questionIndex !== questionIndex) return false;
        return m.content.replace(/\s+/g, " ").trim().toLowerCase() === normalized;
      });
      if (alreadyRecorded) return;

      const lastSent = lastOnAIResponseRef.current.replace(/\s+/g, " ").trim().toLowerCase();
      // Behavioral Qs are spoken verbatim — skip duplicate bank text if already spoken.
      // Coding/whiteboard are often only briefly introduced, so always persist bank text.
      const isUnspokenType = q.type === "CODING" || q.type === "WHITEBOARD";
      if (!isUnspokenType && lastSent === normalized) return;
      if (!isUnspokenType && lastSent.includes(normalized) && normalized.length > 20) return;

      pushTrackedMessage({
        role: "assistant",
        content,
        questionIndex,
      });
      // Surface in live transcript (esp. coding — otherwise never appears).
      if (isUnspokenType || lastSent !== normalized) {
        lastOnAIResponseRef.current = content;
        onAIResponse?.(content);
      }
      scheduleSaveProgressRef.current(questionIndex);
    },
    [getSortedQuestions, pushTrackedMessage, onAIResponse]
  );

  const saveProgress = flushSaveProgress;

  /** Handle JSON messages from relay */
  const handleRelayMessage = useCallback(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (msg: Record<string, any>) => {
      switch (msg.type) {
        case "ready":
          log.info("Session ready:", msg.sessionId);
          setState((s) => ({ ...s, isConnected: true }));
          break;

        case "interrupt":
          interruptPlayback();
          setState((s) => ({ ...s, isProcessing: false }));
          if (chatBufferRef.current.trim()) {
            const text = chatBufferRef.current.trim();
            setState((s) => ({
              ...s,
              aiTranscript: text,
              lastAiQuestion: text || s.lastAiQuestion,
            }));
            chatBufferRef.current = "";
          }
          break;

        case "asr": {
          // Live ASR — prefer the relay's growing turn text; never shrink on shorter partials.
          const results =
            (msg.data?.results as Array<Record<string, unknown>>) || [];
          if (results.length > 0) {
            const text = ((results[0].text as string) || "").trim();
            if (text) {
              const prev = asrBufferRef.current.trim();
              const merged =
                prev && !text.includes(prev) && !prev.includes(text)
                  ? `${prev} ${text}`.replace(/\s+/g, " ").trim()
                  : text.length >= prev.length
                    ? text
                    : prev || text;
              asrBufferRef.current = merged;
              if (isInListeningTurnRef.current || isListeningRef.current) {
                listeningTurnTextRef.current = merged;
              }
              setState((s) => ({ ...s, userTranscript: merged }));
              onTranscript?.(merged, false);
            }
          }
          break;
        }

        case "asr_ended": {
          const incoming =
            (typeof msg.text === "string" && msg.text.trim()) ||
            asrBufferRef.current.trim();

          // Still unmuted and not committing: accumulate into ONE turn — do not persist yet.
          if (
            !commitPendingRef.current &&
            (isListeningRef.current || isInListeningTurnRef.current)
          ) {
            if (incoming) {
              const prev = listeningTurnTextRef.current.trim();
              listeningTurnTextRef.current =
                prev && !prev.includes(incoming)
                  ? `${prev} ${incoming}`.replace(/\s+/g, " ").trim()
                  : incoming.length >= prev.length
                    ? incoming
                    : prev || incoming;
              asrBufferRef.current = listeningTurnTextRef.current;
              setState((s) => ({
                ...s,
                userTranscript: listeningTurnTextRef.current,
              }));
              onTranscript?.(listeningTurnTextRef.current, false);
            }
            break;
          }

          // Mute/Submit commit — one candidate answer for the whole unmute→mute window.
          const finalText =
            incoming ||
            listeningTurnTextRef.current.trim() ||
            asrBufferRef.current;
          commitPendingRef.current = false;
          isInListeningTurnRef.current = false;
          listeningTurnTextRef.current = "";
          if (finalText) {
            markClarificationFromUserText(finalText, "asr");
            const normalized = finalText.replace(/\s+/g, " ").trim().toLowerCase();
            const lastFinal = lastFinalUserTranscriptRef.current;
            const isDuplicateFinal =
              !!lastFinal &&
              lastFinal.text === normalized &&
              Date.now() - lastFinal.at < 15_000;

            if (isDuplicateFinal) {
              log.debug(`Skipping duplicate USER final: "${normalized.slice(0, 60)}..."`);
            } else {
              lastFinalUserTranscriptRef.current = { text: normalized, at: Date.now() };
              // Prefer index frozen at commitTurn (pregen may advance clips immediately).
              const qIdx = activeTurnQuestionIndexRef.current;
              onTranscript?.(finalText, true);
              pushTrackedMessage({
                role: "user",
                content: finalText,
                questionIndex: qIdx,
                timestamp: listeningTurnStartedAtRef.current ?? new Date().toISOString(),
              });
              listeningTurnStartedAtRef.current = null;
              asrBufferRef.current = "";
              chatBufferRef.current = "";
              scheduleSaveProgress(qIdx);
            }
          } else {
            lastUserWasClarificationRef.current = false;
          }
          asrBufferRef.current = "";
          setState((s) => ({
            ...s,
            userTranscript: "",
            isProcessing: !!finalText,
          }));
          resolveCommitTurnWaiter(finalText || "");
          break;
        }

        case "chat": {
          const text = extractText(msg.data);
          if (text) {
            chatBufferRef.current += text;
            setState((s) => ({
              ...s,
              aiTranscript: chatBufferRef.current,
              lastAiQuestion: chatBufferRef.current,
              lastAssistantUtteranceEndedAt: 0,
              isProcessing: false,
            }));
          }
          break;
        }

        case "tts_text": {
          const text = extractText(msg.data);
          if (text) {
            chatBufferRef.current += (chatBufferRef.current ? " " : "") + text;
            setState((s) => ({
              ...s,
              aiTranscript: chatBufferRef.current,
              lastAiQuestion: chatBufferRef.current,
              lastAssistantUtteranceEndedAt: 0,
              isProcessing: false,
            }));
            log.debug(
              `TTS sentence: "${text.slice(0, 80)}..."`
            );
          }
          break;
        }

        case "tts_sentence_end":
          break;

        case "chat_ended":
        case "tts_ended": {
          const fullResponse = chatBufferRef.current.trim();
          chatBufferRef.current = "";
          // Keep last transcript visible until next AI speech (replaced by tts_text/chat)
          setState((s) => ({
            ...s,
            aiTranscript: fullResponse,
            lastAiQuestion: fullResponse || s.lastAiQuestion,
            lastAssistantUtteranceEndedAt:
              msg.type === "tts_ended" ? Date.now() : s.lastAssistantUtteranceEndedAt,
            isProcessing: false,
          }));
          if (fullResponse) {
            // Dedupe: chat_ended and tts_ended often both fire for same response.
            // Use dedicated ref — trackedMessagesRef can lag; normalize for comparison.
            const normalized = fullResponse.replace(/\s+/g, " ").trim();
            const lastSent = lastOnAIResponseRef.current.replace(/\s+/g, " ").trim();
            if (normalized === lastSent) {
              log.debug(`Skipping duplicate ASSISTANT (${msg.type})`);
            } else {
              lastOnAIResponseRef.current = fullResponse;
              onAIResponse?.(fullResponse);
              pushTrackedMessage({
                role: "assistant",
                content: fullResponse,
                questionIndex: activeTurnQuestionIndexRef.current,
              });
              log.debug(
                `Tracked ASSISTANT (${msg.type}): "${fullResponse.slice(0, 60)}..."`
              );
              // Auto-save progress on every message
              scheduleSaveProgress(currentQuestionIndexRef.current);
            }
          }
          break;
        }

        case "session_reconnecting":
          interruptPlayback();
          if (chatBufferRef.current.trim()) {
            const text = chatBufferRef.current.trim();
            const normalized = text.replace(/\s+/g, " ").trim();
            const lastSent = lastOnAIResponseRef.current.replace(/\s+/g, " ").trim();
            if (normalized !== lastSent) {
              lastOnAIResponseRef.current = text;
              onAIResponse?.(text);
              pushTrackedMessage({
                role: "assistant",
                content: text,
                questionIndex: activeTurnQuestionIndexRef.current,
              });
            }
          }
          chatBufferRef.current = "";
          lastOnAIResponseRef.current = "";
          setState((s) => ({ ...s, aiTranscript: "", isProcessing: true }));
          break;

        case "session_reconnected":
          setState((s) => ({ ...s, isProcessing: false }));
          break;

        case "question_change": {
          // Never interrupt on auto transitions — the full acknowledgement +
          // next-question audio should finish playing naturally.
          if (!msg.auto) {
            interruptPlayback();
          }
          const idx = Number(msg.questionIndex);
          const total = Number(msg.totalQuestions);
          if (!Number.isFinite(idx) || !Number.isFinite(total)) break;

          const outgoingIdx = currentQuestionIndexRef.current;

          // Hard client guard: ignore advances that arrive right after a
          // repeat/clarify turn (belt-and-suspenders vs relay bugs).
          if (
            lastUserWasClarificationRef.current &&
            idx !== outgoingIdx &&
            idx === outgoingIdx + 1
          ) {
            log.warn(
              `Blocked client question_change after clarification ` +
                `(wanted Q${idx + 1}, staying on Q${outgoingIdx + 1})`,
            );
            setState((s) => ({
              ...s,
              isTransitioning: false,
              transitionDirection: null,
            }));
            break;
          }
          if (idx !== outgoingIdx) {
            lastUserWasClarificationRef.current = false;
          }

          const pendingAssistant = chatBufferRef.current.trim();
          if (pendingAssistant && outgoingIdx !== idx) {
            const normalized = pendingAssistant.replace(/\s+/g, " ").trim();
            const lastSent = lastOnAIResponseRef.current.replace(/\s+/g, " ").trim();
            if (normalized !== lastSent) {
              lastOnAIResponseRef.current = pendingAssistant;
              onAIResponse?.(pendingAssistant);
              pushTrackedMessage({
                role: "assistant",
                content: pendingAssistant,
                questionIndex: outgoingIdx,
              });
            }
            chatBufferRef.current = "";
          }

          // Persist messages from the previous question immediately
          void flushSaveProgress(idx);

          currentQuestionIndexRef.current = idx;
          // If the mic stayed open across a question boundary (common after coding
          // segments), refresh the turn start + question index so the answer is not
          // stamped with a pre-question timestamp / coding questionId.
          if (isListeningRef.current || isInListeningTurnRef.current) {
            listeningTurnStartedAtRef.current = new Date().toISOString();
            activeTurnQuestionIndexRef.current = idx;
          }
          // Always record the scripted question text (esp. coding — often unspoken).
          if (idx >= 0 && idx < total) {
            recordQuestionEntry(idx);
          }
          setState((s) => ({
            ...s,
            currentQuestionIndex: idx,
            totalQuestions: total,
            isTransitioning: false,
            transitionDirection: null,
          }));
          onQuestionChange?.(idx, total);
          log.info(`Question ${idx + 1}/${total}`);
          break;
        }

        case "transitioning": {
          const dir = (msg.direction as "next" | "previous") ?? "next";
          if (
            lastUserWasClarificationRef.current &&
            dir === "next"
          ) {
            log.warn("Ignoring transitioning:next after clarification turn");
            break;
          }
          if (!msg.auto) {
            interruptPlayback();
            chatBufferRef.current = "";
            setState((s) => ({
              ...s,
              isTransitioning: true,
              transitionDirection: dir,
              isSpeaking: false,
              aiTranscript: "",
              lastAiQuestion: "",
              lastAssistantUtteranceEndedAt: 0,
            }));
          } else {
            setState((s) => ({
              ...s,
              isTransitioning: true,
              transitionDirection: dir,
            }));
          }
          break;
        }

        case "interview_complete":
          log.info("Interview complete, wrapping up");
          if (msg.tokenUsage) {
            tokenUsageRef.current = msg.tokenUsage;
          }
          setState((s) => ({ ...s, isInterviewComplete: true, tokenUsage: msg.tokenUsage }));
          break;

        case "error": {
          const message = (msg.message as string) || "Relay error";
          if (
            isRecoverableRelayErrorMessage(message) &&
            relayConnectorRef.current?.canFailover
          ) {
            log.warn(`Recoverable relay error ignored during failover window: ${message}`);
            break;
          }
          onError?.(message);
          break;
        }

        case "disconnected":
          setState((s) => ({ ...s, isConnected: false }));
          if (relayConnectorRef.current?.canFailover) {
            void relayConnectorRef.current.failover("relay disconnected message");
          }
          break;
      }
    },
    [interruptPlayback, extractText, onTranscript, onAIResponse, onError, onQuestionChange, flushSaveProgress, scheduleSaveProgress, recordQuestionEntry, resolveCommitTurnWaiter, markClarificationFromUserText]
  );

  /** Start capturing microphone audio and sending to relay */
  const startListening = useCallback(async () => {
    if (isListeningRef.current) return;

    try {
      if (audioContextRef.current?.state === "suspended") {
        await audioContextRef.current.resume();
      }
      if (typeof navigator === "undefined" || !navigator.mediaDevices) {
        throw new Error("Microphone access requires a secure (HTTPS) connection.");
      }
      const stream = await navigator.mediaDevices.getUserMedia({
        audio: {
          sampleRate: 16000,
          channelCount: 1,
          echoCancellation: true,
          noiseSuppression: true,
          autoGainControl: true,
        },
      });
      mediaStreamRef.current = stream;

      const ctx = new AudioContext({ sampleRate: 16000 });
      const source = ctx.createMediaStreamSource(stream);
      const processor = ctx.createScriptProcessor(4096, 1, 1);

      source.connect(processor);
      processor.connect(ctx.destination);

      processor.onaudioprocess = (event) => {
        if (!isListeningRef.current) return;
        const connector = relayConnectorRef.current;
        if (!connector?.isReady) return;

        const inputData = event.inputBuffer.getChannelData(0);

        // Compute RMS audio level (float32 range 0..1)
        let sumSq = 0;
        for (let i = 0; i < inputData.length; i++) sumSq += inputData[i] * inputData[i];
        const rms = Math.sqrt(sumSq / inputData.length);
        const level = Math.min(1, rms * 5);
        setState((s) => ({ ...s, audioLevel: level }));

        if (performance.now() < micHoldUntilRef.current) {
          if (rms < BARGE_IN_RMS_THRESHOLD) {
            bargeInFramesRef.current = 0;
            return;
          }

          bargeInFramesRef.current += 1;
          if (bargeInFramesRef.current < BARGE_IN_FRAME_COUNT) {
            return;
          }

          // Sustained near-field speech should still be able to interrupt TTS.
          micHoldUntilRef.current = 0;
        } else {
          bargeInFramesRef.current = 0;
        }

        // Convert float32 to int16 PCM
        const pcm = new Int16Array(inputData.length);
        for (let i = 0; i < inputData.length; i++) {
          pcm[i] = Math.max(-32768, Math.min(32767, inputData[i] * 32768));
        }

        onMicPcmChunkRef.current?.(pcm);

        // Send as hex-encoded string
        const bytes = new Uint8Array(pcm.buffer);
        let hex = "";
        for (let i = 0; i < bytes.length; i++) {
          hex += bytes[i].toString(16).padStart(2, "0");
        }

        connector.sendJson({ type: "audio", data: hex });
      };

      processorRef.current = { processor, source, ctx };
      isListeningRef.current = true;
      listeningTurnStartedAtRef.current = new Date().toISOString();
      isInListeningTurnRef.current = true;
      commitPendingRef.current = false;
      listeningTurnTextRef.current = "";
      asrBufferRef.current = "";
      activeTurnQuestionIndexRef.current = currentQuestionIndexRef.current;
      onListeningStartRef.current?.();
      setState((s) => ({ ...s, isListening: true, userTranscript: "" }));
    } catch (error) {
      const msg =
        error instanceof Error ? error.message : "Microphone access failed";
      onError?.(msg);
    }
  }, [onError]);

  /** Stop capturing microphone (does not persist — commitTurn / asr_ended does). */
  const stopListening = useCallback(() => {
    isListeningRef.current = false;

    if (processorRef.current) {
      const { processor, source, ctx } = processorRef.current;
      processor.disconnect();
      source.disconnect();
      ctx.close();
      processorRef.current = null;
    }

    if (mediaStreamRef.current) {
      mediaStreamRef.current.getTracks().forEach((t) => t.stop());
      mediaStreamRef.current = null;
    }

    setState((s) => ({ ...s, isListening: false, audioLevel: 0 }));
  }, []);

  /** Request transition to the next question */
  const nextQuestion = useCallback(() => {
    relayConnectorRef.current?.sendJson({ type: "next_question" });
  }, []);

  /** Request transition back to the previous question */
  const previousQuestion = useCallback(() => {
    relayConnectorRef.current?.sendJson({ type: "prev_question" });
  }, []);

  /**
   * Sync client-managed question index (e.g. pregenerated clip flow).
   * Does not ask the relay to advance — only stamps subsequent USER saves.
   */
  const setQuestionIndex = useCallback((idx: number) => {
    if (!Number.isFinite(idx) || idx < 0) return;
    currentQuestionIndexRef.current = idx;
    setState((s) =>
      s.currentQuestionIndex === idx ? s : { ...s, currentQuestionIndex: idx },
    );
  }, []);

  /** Commit the current turn; resolves when relay emits asr_ended (or timeout).
   *  Relay returns a single asr_ended covering all speech since unmute. */
  const commitTurn = useCallback((): Promise<string> => {
    // Snapshot interim text in case relay commit is empty but user spoke.
    if (asrBufferRef.current.trim()) {
      listeningTurnTextRef.current = asrBufferRef.current.trim();
    }
    const localText =
      listeningTurnTextRef.current.trim() || asrBufferRef.current.trim();
    // Set clarification guard before relay can emit question_change (Q1 race).
    if (localText) {
      markClarificationFromUserText(localText, "commit");
    } else {
      lastUserWasClarificationRef.current = false;
    }
    // Freeze question index before UI advances (pregenerated clip flow).
    activeTurnQuestionIndexRef.current = currentQuestionIndexRef.current;
    isInListeningTurnRef.current = false;
    commitPendingRef.current = true;

    // Replace any prior in-flight waiter (rapid double-submit).
    if (commitTurnWaiterRef.current) {
      clearTimeout(commitTurnWaiterRef.current.timer);
      commitTurnWaiterRef.current.resolve(
        listeningTurnTextRef.current.trim() || asrBufferRef.current.trim() || "",
      );
      commitTurnWaiterRef.current = null;
    }

    return new Promise<string>((resolve) => {
      const timer = setTimeout(() => {
        commitTurnWaiterRef.current = null;
        commitPendingRef.current = false;
        const fallback =
          listeningTurnTextRef.current.trim() ||
          asrBufferRef.current.trim() ||
          "";
        log.warn(
          `commitTurn timed out after ${COMMIT_TURN_TIMEOUT_MS}ms — using local fallback (${fallback.length} chars)`,
        );
        // Persist local snapshot if relay never acked (avoid losing the answer).
        if (fallback) {
          markClarificationFromUserText(fallback, "commit");
          const normalized = fallback.replace(/\s+/g, " ").trim().toLowerCase();
          const lastFinal = lastFinalUserTranscriptRef.current;
          const isDuplicateFinal =
            !!lastFinal &&
            lastFinal.text === normalized &&
            Date.now() - lastFinal.at < 15_000;
          if (!isDuplicateFinal) {
            lastFinalUserTranscriptRef.current = { text: normalized, at: Date.now() };
            const qIdx = activeTurnQuestionIndexRef.current;
            onTranscript?.(fallback, true);
            pushTrackedMessage({
              role: "user",
              content: fallback,
              questionIndex: qIdx,
              timestamp: listeningTurnStartedAtRef.current ?? new Date().toISOString(),
            });
            listeningTurnStartedAtRef.current = null;
            scheduleSaveProgress(qIdx);
          }
        }
        asrBufferRef.current = "";
        listeningTurnTextRef.current = "";
        resolve(fallback);
      }, COMMIT_TURN_TIMEOUT_MS);

      commitTurnWaiterRef.current = { resolve, timer };
      relayConnectorRef.current?.sendJson({ type: "commit_turn" });
    });
  }, [onTranscript, scheduleSaveProgress, pushTrackedMessage, markClarificationFromUserText]);

  /** Send a text message through the relay (treated like a voice utterance).
   *  The caller is responsible for adding the message to the UI display;
   *  this method only tracks it for server persistence and sends it to the relay. */
  const sendTextMessage = useCallback(
    (text: string) => {
      const trimmed = text.trim();
      if (!trimmed) return;
      const connector = relayConnectorRef.current;
      if (!connector?.isReady) return;

      activeTurnQuestionIndexRef.current = currentQuestionIndexRef.current;
      pushTrackedMessage({ role: "user", content: trimmed, source: "chat" });
      connector.sendJson({ type: "text_input", content: trimmed });
      void saveProgress(currentQuestionIndexRef.current);
    },
    [saveProgress],
  );

  /** Send code editor content to the relay for agent context */
  const sendCodeUpdate = useCallback((content: string, language: string) => {
    latestCodeUpdateRef.current = { content, language };
    relayConnectorRef.current?.sendJson({ type: "code_update", content, language });
  }, []);

  /** Send whiteboard image to the relay for agent context */
  const sendWhiteboardUpdate = useCallback((imageDataUrl: string) => {
    latestWhiteboardUpdateRef.current = imageDataUrl;
    relayConnectorRef.current?.sendJson({ type: "whiteboard_update", imageDataUrl });
  }, []);

  /** Save remaining tracked messages and complete the session */
  const saveAndComplete = useCallback(async () => {
    // Flush any pending buffers before saving
    const pendingAsrText = asrBufferRef.current.trim();
    if (pendingAsrText) {
      pushTrackedMessage({ role: "user", content: pendingAsrText });
      asrBufferRef.current = "";
    }
    const pendingChatText = chatBufferRef.current.trim();
    if (pendingChatText) {
      pushTrackedMessage({
        role: "assistant",
        content: pendingChatText,
        questionIndex: activeTurnQuestionIndexRef.current,
      });
      chatBufferRef.current = "";
    }

    const messages = trackedMessagesRef.current;
    if (!sessionId) return;

    log.info(
      `Saving ${messages.length} remaining messages and completing session`,
    );

    try {
      // Always complete — empty transcript must not skip DB status flip
      await fetch("/api/voice/save", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        keepalive: true, // Ensure request finishes even if browser navigates away
        body: JSON.stringify({
          sessionId,
          messages,
          complete: true,
          tokenUsage: tokenUsageRef.current,
        }),
      });
    } catch (err) {
      log.error("Failed to save voice data:", err);
      // Fallback dedicated complete endpoint (beacon-compatible)
      try {
        await fetch("/api/session/complete", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ sessionId }),
          keepalive: true,
        });
      } catch (completeErr) {
        log.error("Fallback session complete failed:", completeErr);
      }
    }
  }, [sessionId]);

  /** Disconnect, save messages, and clean up everything */
  const disconnect = useCallback(async () => {
    setState((s) => ({ ...s, isSaving: true }));
    try {
      await saveAndComplete();
    } finally {
      cleanup();
      // isSaving is reset by cleanup
    }
  }, [saveAndComplete, cleanup]);

  return {
    ...state,
    connect,
    disconnect,
    startListening,
    stopListening,
    nextQuestion,
    previousQuestion,
    setQuestionIndex,
    commitTurn,
    sendTextMessage,
    sendCodeUpdate,
    sendWhiteboardUpdate,
    interruptPlayback,
    mediaStreamRef,
  };
}
