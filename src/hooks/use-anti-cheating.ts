...
export function useAntiCheating({
  ...
  useEffect(() => {
    ...
    const recordDeparture = () => {
      // Ignore departures within the first 15 seconds of enabling (allows users to click 'Hide' on the screen share dialog)
      if (Date.now() - mountTime < 15000) return;
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (clipboard && origWriteText) clipboard.writeText = origWriteText;
