...
export const questionRouter = router({
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (role === "EDITOR" && interviewUserId !== ctx.user.id) {
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { data: question } = await ctx.supabase
        .from("questions")
        .select("id, interviewId, text, order")
        .eq("id", id)
        .single();
      ...
      // Video regeneration is opted-in from Content (confirm dialog → /api/ai/pregenerate).
      return updated;
    }),
