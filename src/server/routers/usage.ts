...
import { getInterviewTypeLabel } from "@/lib/interview-credits";
  ...
  templateTransactions: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const { data: interviews } = await supabaseAdmin
        .from("interviews")
        .select("id, title, projectId, chatEnabled, voiceEnabled, videoEnabled, avatarMode, is_voice_only, createdAt")
        .in("projectId", pIds)
        .gte("createdAt", range.start.toISOString())
        .lte("createdAt", range.end.toISOString())
        .order("createdAt", { ascending: false })
      ...
      return (interviews ?? []).map((iv) => {
        const type = getInterviewTypeLabel({
          voiceEnabled: iv.voiceEnabled as boolean | null,
          videoEnabled: iv.videoEnabled as boolean | null,
          avatarMode: (iv as { avatarMode?: string | null }).avatarMode,
          is_voice_only: (iv as { is_voice_only?: boolean | null }).is_voice_only,
        });
        return {
          id: iv.id as string,
