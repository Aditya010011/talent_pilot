  ...
  invite: protectedProcedure
    .input(
      z.object({
        ...
        role: z.enum(["ACCOUNT_ADMIN", "EDITOR", "VIEWER"]).default("EDITOR"),
      }),
    )
    .mutation(async ({ ctx, input }) => {
      ...
      if (membership.role === "ACCOUNT_ADMIN" && input.role === "ACCOUNT_ADMIN") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Only the owner (SYSTEM_ADMIN) can invite members as Admin",
        });
      }
  ...
  updateRole: protectedProcedure
    .input(
      z.object({
        organizationId: z.string(),
        userId: z.string(),
        role: z.enum(["ACCOUNT_ADMIN", "EDITOR", "VIEWER"]),
      }),
    )
    .mutation(async ({ ctx, input }) => {
      ...
      if (
        callerMembership.role === "ACCOUNT_ADMIN" &&
        input.role === "ACCOUNT_ADMIN" &&
        targetMembership.role !== "ACCOUNT_ADMIN"
      ) {
        throw new TRPCError({
          code: "FORBIDDEN",
