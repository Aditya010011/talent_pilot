...
import { findMatchingAvatarPreset } from "@/lib/avatar-voices";
import { normalizePregeneratedStore } from "@/lib/pregenerated-videos";
...
/** Legacy string or { name, description } — normalized before save. */
const cvCriterionSchema = z.union([
  z.string(),
  z.object({
    name: z.string(),
...
export const interviewRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      let query = ctx.supabase
        .from("interviews")
        .select(
          "*, questions(id), sessions(id, status), candidates(id, sessionId), project:projects!inner(id, organizationId)",
        )
      ...
      const interviews = rows.slice(0, limit).map((i) => {
        const sessions = (i.sessions as { id: string; status: string }[]) ?? [];
        const candidates = (i.candidates as { id: string; sessionId: string | null }[]) ?? [];

        const completedSessionsCount = sessions.filter(
          (s) => s.status === "COMPLETED"
        ).length;

        const linkedSessionIds = new Set(candidates.map((c) => c.sessionId).filter(Boolean));
        const walkInsCount = sessions.filter((s) => !linkedSessionIds.has(s.id)).length;
        const totalInvitedCount = candidates.length + walkInsCount;
        ...
        return {
          ...
          _count: {
            ...
            invitedCandidates: totalInvitedCount,
          },
          questions: undefined,
          sessions: undefined,
  ...
  create: protectedProcedure
    .input(
      z.object({
        ...
        assessmentCriteria: z
          ...
          .optional(),
        cvAssessmentCriteria: z.array(cvCriterionSchema).optional(),
        cvJdAlignmentCriteria: z.array(cvCriterionSchema).optional(),
        chatEnabled: z.boolean().default(true),
        voiceEnabled: z.boolean().default(false),
        videoEnabled: z.boolean().default(false),
        avatarMode: z.enum(["none", "static", "simli", "vidu"]).default("none"),
        whiteboardEnabled: z.boolean().default(false),
        codeEnabled: z.boolean().default(false),
        aiName: z.string().default("Inluwa"),
        aiTone: z
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { data: orgFlags } = await ctx.supabase
        .from("organizations")
        .select("interviewEnabled")
        .eq("id", project.organizationId)
        .single();
      if (orgFlags?.interviewEnabled === false) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Interview is not enabled for this organization.",
        });
      }

      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const { projectId: _pid, isVoiceOnly, multilingualEnabled, cvAssessmentCriteria, cvJdAlignmentCriteria, ...rest } = input;
      const { data: interview, error } = await ctx.supabase
        .from("interviews")
        .insert({
          ...rest,
          ...(cvAssessmentCriteria !== undefined
            ? { cvAssessmentCriteria: normalizeCvCriteria(cvAssessmentCriteria) }
            : {}),
          ...(cvJdAlignmentCriteria !== undefined
            ? { cvJdAlignmentCriteria: normalizeCvCriteria(cvJdAlignmentCriteria) }
            : {}),
          is_voice_only: isVoiceOnly,
          multilingualEnabled: isVoiceOnly ? false : multilingualEnabled,
          projectId,
          userId: ctx.user.id,
          requireInvite: true,
        })
  ...
  update: protectedProcedure
    .input(
      z.object({
        ...
        avatarMode: z.enum(["none", "static", "simli", "vidu"]).optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (result.role === "EDITOR" && result.interview.userId !== ctx.user.id) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message:
      ...
      assertMinRole(result.role, "EDITOR");

      const { id, isVoiceOnly, multilingualEnabled, cvAssessmentCriteria, cvJdAlignmentCriteria, ...data } = input;
      const voiceOnly = isVoiceOnly ?? result.interview.is_voice_only;
      
      let pregeneratedVideos = result.interview.pregenerated_videos;
      if (data.avatarImageUrl !== undefined || data.avatarVoice !== undefined) {
        const imageUrl = data.avatarImageUrl !== undefined ? data.avatarImageUrl : result.interview.avatarImageUrl;
        const voice = data.avatarVoice !== undefined ? data.avatarVoice : result.interview.avatarVoice;
        const preset = findMatchingAvatarPreset(imageUrl, voice);
        
        if (preset?.listeningVideoPath) {
          const store = normalizePregeneratedStore(pregeneratedVideos);
          store.headNod = {
            activeVersion: 1,
            versions: [
              {
                v: 1,
                url: preset.listeningVideoPath,
                text: "Listening",
                createdAt: new Date().toISOString()
              }
            ]
          };
          pregeneratedVideos = store as any;
        }
      }

      const updateData = {
        ...data,
        ...(isVoiceOnly !== undefined ? { is_voice_only: isVoiceOnly } : {}),
        ...(multilingualEnabled !== undefined
          ? { multilingualEnabled: voiceOnly ? false : multilingualEnabled }
          : {}),
        ...(cvAssessmentCriteria !== undefined
          ? { cvAssessmentCriteria: normalizeCvCriteria(cvAssessmentCriteria) }
          : {}),
        ...(cvJdAlignmentCriteria !== undefined
          ? { cvJdAlignmentCriteria: normalizeCvCriteria(cvJdAlignmentCriteria) }
          : {}),
        ...(pregeneratedVideos !== result.interview.pregenerated_videos
          ? { pregenerated_videos: pregeneratedVideos }
          : {}),
      };
      const { data: interview, error } = await ctx.supabase
        .from("interviews")
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { data: newInterview, error } = await ctx.supabase
        ...
        .insert({
          ...
          avatarMode: (source as { avatarMode?: string }).avatarMode ?? "none",
          userId: ctx.user.id,
          ...
          multilingualEnabled: (source as { multilingualEnabled?: boolean }).multilingualEnabled ?? false,
          ...
          whiteboardEnabled: (source as { whiteboardEnabled?: boolean }).whiteboardEnabled ?? false,
          codeEnabled: (source as { codeEnabled?: boolean }).codeEnabled ?? false,
          cvAssessmentCriteria: (source as { cvAssessmentCriteria?: unknown }).cvAssessmentCriteria ?? null,
          cvJdAlignmentCriteria: (source as { cvJdAlignmentCriteria?: unknown }).cvJdAlignmentCriteria ?? null,
          avatarImageUrl: (source as { avatarImageUrl?: string | null }).avatarImageUrl ?? null,
          avatarVoice: (source as { avatarVoice?: string | null }).avatarVoice ?? null,
          is_voice_only: (source as { is_voice_only?: boolean }).is_voice_only ?? false,
        })
        .select()
        .single();
  ...
  dashboardStats: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      console.log("[dashboardStats] Starting query with input:", JSON.stringify(input));
      const { data: memberships } = await ctx.supabase
        .from("organization_members")
        .select("workspaceId")
        .eq("userId", ctx.user.id);
      ...
      console.log("[dashboardStats] Memberships fetched, orgIds:", orgIds);
      if (orgIds.length === 0) {
        console.log("[dashboardStats] No organization memberships found, returning empty dashboard");
      ...
      if (input?.projectId) {
        ...
        console.log(`[dashboardStats] Input projectId ${input.projectId} access check result:`, ok);
        ...
        console.log("[dashboardStats] Resolved accessible projectIds:", projectIds);
      }
      if (projectIds.length === 0) {
        console.log("[dashboardStats] No accessible project IDs, returning empty dashboard");
        return emptyDashboard();
      }

      // Always fetch all interviews in project — date filters apply to sessions/questions (activity data)
      const { data: interviews } = await ctx.supabase
        .from("interviews")
        .select("id, createdAt")
        .in("projectId", projectIds);
      const interviewIds = (interviews ?? []).map((i: { id: string }) => i.id);
      console.log(`[dashboardStats] Loaded ${interviewIds.length} interviews for projectIds`);
      if (interviewIds.length === 0) {
        console.log("[dashboardStats] No interviews found, returning empty dashboard");
        return emptyDashboard();
      }

      let questionsCountQuery = ctx.supabase
        .from("questions")
        .select("id", { count: "exact", head: true })
        .in("interviewId", interviewIds);
      if (input?.startDate) {
        questionsCountQuery = questionsCountQuery.gte("createdAt", input.startDate);
      }
      if (input?.endDate) {
        questionsCountQuery = questionsCountQuery.lte("createdAt", input.endDate);
      }
      const { count: totalQuestions } = await questionsCountQuery;
      console.log("[dashboardStats] Fetched total questions count:", totalQuestions);
      ...
      let sessionsQuery = ctx.supabase
        ...
        .in("interviewId", interviewIds);
      if (input?.startDate) {
        sessionsQuery = sessionsQuery.gte("createdAt", input.startDate);
      }
      if (input?.endDate) {
        sessionsQuery = sessionsQuery.lte("createdAt", input.endDate);
      }
      const { data: sessions } = await sessionsQuery;
      const allSessions = sessions ?? [];
      console.log(`[dashboardStats] Loaded ${allSessions.length} sessions within filter bounds`);

      let questionsQuery = ctx.supabase
        .from("questions")
        .select("id, type, createdAt")
        .in("interviewId", interviewIds);
      if (input?.startDate) {
        questionsQuery = questionsQuery.gte("createdAt", input.startDate);
      }
      if (input?.endDate) {
        questionsQuery = questionsQuery.lte("createdAt", input.endDate);
      }
      const { data: questionRows } = await questionsQuery;
      console.log(`[dashboardStats] Loaded ${questionRows?.length ?? 0} individual question rows within bounds`);
      ...
      // Daily time series (dynamic date range)
      ...
      // Build standard date iterator strings safely avoiding timezone shift increments
      ...
      return {
        totalInterviews: (interviews ?? []).filter((i: any) => {
          if (input?.startDate && i.createdAt < input.startDate) return false;
          if (input?.endDate && i.createdAt > input.endDate) return false;
          return true;
        }).length,
        totalSessions,
