...
export type MemberRole = "SYSTEM_ADMIN" | "ACCOUNT_ADMIN" | "EDITOR" | "VIEWER";
...
export async function getOrgMembership(
  ...
  if (await isSystemAdmin(supabase, userId)) {
    return { role: "SYSTEM_ADMIN" as const };
  }
...
/**
 * Check if a user holds SYSTEM_ADMIN role in ANY organization.
 */
...
export async function hasProjectAccess(
  ...
  // Retrieve the organizationId of the project
  ...
  if (project?.organizationId) {
    ...
    // Org admins and system admins always have access to all projects
    if (membership && (membership.role === "ACCOUNT_ADMIN" || membership.role === "SYSTEM_ADMIN")) {
      return true;
    }
  }
