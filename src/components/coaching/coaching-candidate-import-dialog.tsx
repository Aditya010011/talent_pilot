...
interface CoachingCandidateImportDialogProps {
  trainingId: string;
  open: boolean;
...
function parseExcel(data: ArrayBuffer): ParsedCandidate[] {
  ...
  for (let i = headerIndex + 1; i < allRows.length; i++) {
    ...
    if (email) {
      const emailResult = z.string().email().safeParse(email.toLowerCase());
      if (emailResult.success) {
        candidate.email = emailResult.data;
      }
    }
...
export function CoachingCandidateImportDialog({
  trainingId,
  open,
  onOpenChange,
  onImported,
}: CoachingCandidateImportDialogProps) {
          ...
          message = parsed
            .map((p) => {
              if (p.path && p.path.length > 0) {
                if (p.path[0] === "candidates" && typeof p.path[1] === "number") {
                  return `Row ${p.path[1] + 1}: ${p.message}`;
                }
              }
              return p.message;
            })
  ...
  }, [candidates, trainingId, bulkCreate]);
