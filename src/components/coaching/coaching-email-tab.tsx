...
const TEMPLATE_VARS = [
  ...
  { label: "Training Title", value: "{{interview_title}}" },
...
interface CoachingEmailTabProps {
  trainingId: string;
  training: any;
  isAdmin: boolean;
}

export function CoachingEmailTab({ trainingId, training, isAdmin }: CoachingEmailTabProps) {
  const isViewer = !isAdmin;
  const { toast } = useToast();
  const utils = trpc.useUtils();

  const templateQuery = trpc.coachingEmailInvite.getTemplate.useQuery({
    trainingId,
  });
  const saveMutation = trpc.coachingEmailInvite.saveTemplate.useMutation({
    onSuccess: () => {
      utils.coachingEmailInvite.getTemplate.invalidate({ trainingId });
  ...
  const sendOneMutation = trpc.coachingEmailInvite.sendToCandidate.useMutation({
    onSuccess: () => {
      utils.coachingEmailInvite.getSendLogs.invalidate({ trainingId });
  ...
  const sendLogs = trpc.coachingEmailInvite.getSendLogs.useQuery({
    trainingId,
    limit: 100,
  });
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileName = `logo_c_${trainingId}.${fileExt}`;
                  ...
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ 
                      __html: (effectiveBody || "<em>Default template will be used</em>")
                        ...
                        .replace(/\{\{interview_title\}\}/gi, training?.title || "Training Title")
                        .replace(/\{\{interview_description\}\}/gi, training?.description || "")
                        .replace(/\{\{invite_link\}\}/gi, `<a href="#" style="color:#7c3aed;text-decoration:underline;">https://app.inluwa.com/c/invite/${previewCandidate?.inviteToken || "xyz123"}</a>`)
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Set the dates during which the invited candidates can access the training.</CardDescription>
            </CardHeader>
            ...
            <CardContent>
              {candidates.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Add candidates in the Candidates tab to send invitations.
                  </p>
                </div>
              )}
            </CardContent>
